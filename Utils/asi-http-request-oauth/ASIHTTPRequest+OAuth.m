...
- (NSString *)oauthRequestParameterString:(NSArray *)oauthParameters
{
    ...
    for (NSDictionary *param in postBodyParameters)
        [parameters addObject:[NSDictionary dictionaryWithObjectsAndKeys:[[param objectForKey:@"key"] encodeForURL], @"key",[self encode:[param objectForKey:@"value"] dataEncodeForBaseString:NO], @"value", nil]];
    ...
    for (NSDictionary *parameter in parameters)
        [parameterStrings addObject:[NSString stringWithFormat:@"%@%%3D%@", [parameter objectForKey:@"key"] ,[self encode: [parameter objectForKey:@"value"] dataEncodeForBaseString:YES]]];

    return [parameterStrings componentsJoinedByString:@"%26"];
}

-(NSString*)encode:(id)obj dataEncodeForBaseString:(BOOL)encode{
    if ([obj isKindOfClass:[NSData class]]) {
        if (encode) {
            NSData * data=obj;
            return [data encodeForOauthBaseString];
        }else{
            return obj;
        }
    }else {
        NSString * str=[obj description];
        return [str encodeForURL];
    }
                            ...
                            usingMethod:(ASIOAuthSignatureMethod)signatureMethod
{
    ...
    NSString *baseString = [NSString stringWithFormat:@"%@&%@&%@", [[self requestMethod] uppercaseString], [baseStringURI encodeForURL], requestParameterString];
...
-(NSDictionary*)accessTokenWithUrl:(NSString *)url username:(NSString *)username password:(NSString *)password clientIdentifier:(NSString *)clientIdentifier secret:(NSString *)clientSecret usingMethod:(ASIOAuthSignatureMethod)signatureMethod{
